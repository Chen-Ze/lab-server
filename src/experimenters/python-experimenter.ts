...
export const pythonExperimenter = async (recipe: PythonSimpleRecipe, subsequence: WrappedRecipe[], onData: (data: RawDataRow) => void, onHalt: ISignal, controller: Controller) => {
    let haltFlag = false;
    ...
    await new Promise((resolve, reject) => {
        const pythonProcess = spawn('python', recipe.command.split(" ").slice(1));
